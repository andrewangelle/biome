...
impl Rule for NoEmptyBlock {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        match query {
            JsCatchClause(clause) => check_catch_block(clause, options),
            _ => check_block(query),
        }
    }
...
fn is_empty(query: &Query) -> bool {
    ...
    match query {
        ...
        JsCatchClause(clause) => {
            let body = clause.body().unwrap();
            body.statements().len() == 0
        }
    }
}
...
fn check_catch_block(clause: &JsCatchClause, options: &NoEmptyBlockOptions) -> Option<TextRange> {
    let body = clause.body().unwrap();
    let is_empty = body.statements().len() == 0;
    let has_comments = body.syntax().has_comments_descendants();
...
impl VisitNode<JsonLanguage> for NoEmptyBlockOptions {
